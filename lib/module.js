...
// Initializes single noth module
	...
	return new Promise((resolve, reject) => {
		...
		loadModule(type, name, defaultConf)
			...
			.catch((err) => {
				...
				// Get the alternative conf options
		...
		if (nothModule) {
			...
			startFn(options)
				...
				.then((loadedModule) => {
					...
					// Save the used conf options for loading the module
					!conf[type] && (conf[type] = {})
					conf[type][name] = options

					// Save the loaded module
					!conf.modules && (conf.modules = {})
					conf.modules[name] = loadedModule
